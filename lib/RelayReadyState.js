...
var RelayReadyState = function () {
  ...
  RelayReadyState.prototype._mergeState = function _mergeState(nextReadyState, newEvents) {
    ...
    require('fbjs/lib/resolveImmediate')(function () {
      ...
      const network_events = _this._readyState.events
      const latestEvent = network_events[network_events.length -1]
      //console.log(`I'm the broker of GraphQLQuery and UI Components(RelayRenderer/RelayContainer).  last network event received is: ${latestEvent['type']}`)
