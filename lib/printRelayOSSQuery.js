...
function printRelayOSSQuery(node) {
 ...
 //bchen: $ajax.send(queryString, variables)
  const queryString = [queryText].concat(fragmentTexts).join(newLine.length ? newLine : ' ')
  //console.log(`************ relay is sending query to server \n ${queryString}`)
  return {
    text: queryString,
    variables: variables
  };
