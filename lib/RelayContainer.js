...
//****************************************************************************************************************************************************
    ...
    RelayContainer.prototype._initialize = function _initialize(props, context, propVariables, prevVariables) {
      ...
      var nextVariables = rawVariables;
      if(componentName === 'StoreContainer'){
        // console.log(`>>>>>>>>>>>> resturn ${JSON.stringify(rawVariables)}`)
      }
      if (prepareVariables) {
       ...
       if(componentName === 'StoreContainer'){
        // console.log(`<<<<<<<<<< nextVariables ${JSON.stringify(nextVariables)}`)
      }
...
function create(Component, spec) {
  ...
  ContainerConstructor.getFragment = function (fragmentName, variableMapping) {
    var fragmentBuilder = fragments[fragmentName];
    if (!fragmentBuilder) {
      require('fbjs/lib/invariant')(false, '%s does not have fragment `%s`.  check the !!parent component!! which calls '+containerName+'.getFragment(). Available ' + 'fragments names: %s', containerName, fragmentName, fragmentNames.map(function (name) {
        return '`' + name + '`';
      }).join(', '));
    }
...
function validateFragmentProp(componentName, fragmentName, fragment, prop, prevVariables) {
  ...
  var hasFragemnt = require('./RelayFragmentPointer').hasFragment(prop, fragment) 
  var hasFragmentData = hasFragemnt || !!prevVariables && require('fbjs/lib/areEqual')(prevVariables, variables);
  if (!hasFragmentData) {
