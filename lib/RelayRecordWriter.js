...
var RelayRecordWriter = function () {
  ...
  RelayRecordWriter.prototype.deleteRecord = function deleteRecord(dataID) {
    console.log(`________________deleting record  ${dataID}`)
  ...
  RelayRecordWriter.prototype.putLinkedRecordIDs = function putLinkedRecordIDs(parentID, storageKey, recordIDs) {
    var _storeRecords  = this._records; //put it into closure
    var parent = this._getRecordForWrite(parentID);
    ...
    var records = recordIDs.map(function (recordID) {
        var storeRecords = _storeRecords;
        if (parentID.lastIndexOf('event:') !== -1 && storageKey === 'competitors') {
          //console.log(`****************************** about to reset: parentID:${parentID}  storageKey: ${storageKey}  resetIDs:${JSON.stringify(recordIDs)}`);
        }
        return require('./RelayRecord').create(recordID);
    });
    //*************************************************************************************** */
    // on event betscreen, we fetched all competitors (competitors: object[7])
    // and after going back to lobby, we refetch carousel,
    // parentId : event:101953, parent is  this._records['event:101953]
    // 
    /*
    previously, 
    this._records['event:101953] : object {
         origId: 101953
         competitors: Array[7]
    }

    Now (after the next line):
    this._records['event:101953] : object {
         origId: 101953
         competitors: Array[3]
    }
     */
    parent[storageKey] = records;  // this is a blatant data override!
    //*************************************************************************************** */
    if (!this._isOptimisticWrite && this._cacheWriter) {
