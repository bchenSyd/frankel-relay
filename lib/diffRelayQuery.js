...
function diffRelayQuery(root, store, queryTracker) {
  ...
  var tmp = visitor.getSplitQueries();
  var result = queries.concat(tmp);
  if (result.length > 0) {
    //console.log(`!!there are some thing missing in the store. I'll have to ask server for it...`)
  }
  return result;
}
...
var RelayDiffQueryBuilder = function () {
  ...
  RelayDiffQueryBuilder.prototype.visitField = function visitField(node, path, _ref2) {
    ...
    var diffResult = undefined
    // default field diffing algorithm
    if (!node.canHaveSubselections()) {
      diffResult = [this.diffScalar(node, dataID),'diffScalar'];
      ...
      diffResult = [this.diffConnection(node, path, dataID),'diffConnection'];
    } else if (node.isPlural()) {
      diffResult = [this.diffPluralLink(node, path, dataID),'diffPluralLink'];
    } else {
       //they query AST node doesn't have __storageKey__ yet at this point;
      
      
      //within diffLink
      //1. build __storageKey__ for that AST node; use Schema + isCoreArgs(callParas)
      //2. look up __recordStore trying to find a field with __storageKey__
      diffResult = [this.diffLink(node, path, dataID),'diffLink']; 
    }
    if(!!diffResult[0] && !!diffResult[0].diffNode){
        // remember __storageKey__ === schema() (filed Type) +  isCoreArgs(callingVariables)
        var storageKey = diffResult[0].diffNode.__storageKey__
        if(storageKey.lastIndexOf('92149') !== -1){
           
           var a =1+2; 
        }
        console.log(`--------------------  ${diffResult[1]}: failed to find [${diffResult[0].diffNode.__storageKey__}] from store`) 
    }else{
        //console.log(`--------------------  ${diffResult[1]}: found ${node.getSchemaName()} from record store: ${dataID}.`) 
    }
...
//************************************************************************** */
//reconcile , combine all diff children + myslef => return to parent
...
//************************************************************************** */
    ...
    if (this._queryTracker && trackedNode && !(trackedNode instanceof require('./RelayQuery').Fragment)) {
      //! bchen: we never got here. don't know how to get here
    ...
    if (linkedIDs === undefined) {
      ...
      var _ret = function () {
        ...
        linkedIDs.forEach(function (itemID) {
          if (itemID.lastIndexOf('event:') !== -1) {
            //console.log(`********************************   diffing ${itemID}  begins...`)  // jira#9004
          }
          ...
          if (itemState) {
            ...
            if (itemState.diffNode) {
              if (itemID.lastIndexOf('event:') !== -1) {
                //console.log(`________________diffing ${itemID}  returns something`)  // jira#9004
              }
