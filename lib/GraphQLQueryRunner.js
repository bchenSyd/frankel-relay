  ...
  storeData.getTaskQueue().enqueue(function () {
    ...
    if (fetchMode === require('./RelayFetchMode').CLIENT) {
       require('fbjs/lib/forEachObject')(querySet, function (query) {
        if (query) {
          // 1. query is the graphql query built in RelayContainer._createQuerySetAndFragmentPointers, after user call setVariables
          // 2. query is diffed against the record storeData ,to make sure we are still keeping the fetched query data last time
          // 3. haven't figured out why we pass in queryTracker. it's never used.....
          //console.log(`graphqlQueryRunner diffRelayQuery for ${query.printQueryNode()} is about to start.........`)
