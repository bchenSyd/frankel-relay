/*
this file is used to put returned response data into relay store. It's another tree traverse. 
key part is  
```
...
var RelayQueryWriter = function (_RelayQueryVisitor) {
  ...
  RelayQueryWriter.prototype.visitField = function visitField(field, state) {
    ...
    //visiting  competitor
    var competitorID = '8735760';
    if (recordID === competitorID) {
      //var currentVal = this._store._records[competitorID]
      //console.log(`***************************************** triage: about to  write competior[${competitorID}] into store    <====== ${JSON.stringify(currentVal)}   11111111111111111111111111`)
    }
   
    //visiting event
    if (recordID.lastIndexOf('event:') !== -1) {
      //console.log(` ***********************************************   triage event ${recordID}:${serializationKey} and about to call _writeXXX  scalar: _writeScalar; object:_writeLink; []:_writePluralLink`)
    }
  ...
  RelayQueryWriter.prototype._writeScalar = function _writeScalar(field, state, recordID, nextValue) {
    var storageKey = field.getStorageKey();
    if (recordID.lastIndexOf('event:') !== -1) {
      //console.log('event scalar property is saved here...')
    }
    var competitorID = '8735297'; 
    if (recordID === competitorID) {
      //console.log(`*****************************************   writing competior[${competitorID}]:${storageKey} value ${nextValue} into store!!    <====== ${JSON.stringify(currentVal)}`)
    }
    var prevValue = this._store.getField(recordID, storageKey);
...
// event:101953 { competitors:[  {}, {}]}
// competitors is pluralLink (Object Array)
  RelayQueryWriter.prototype._writePluralLink = function _writePluralLink(field, state, recordID, fieldData) {
    ...
    //jira#8857 Marky Mark issue 
    if (recordID.lastIndexOf('101953') !== -1 && storageKey === 'competitors') {
      const relay_store = this._store;
      console.log(`*************   events[${recordID}] Array properties :${storageKey} is created here...  1111111111`)
    }
    ...
    nextLinkedIDs.forEach(function (nextLinkedID) {
      ...
      if (itemData) {
        if (recordID.lastIndexOf('event:') !== -1) {
          //console.log(`*************   events[${recordID}].${storageKey}.[${nextLinkedID}] is created here...`)
        }
  ...
  RelayQueryWriter.prototype._writeLink = function _writeLink(field, state, recordID, fieldData) {
    ...
    var path = require('./RelayQueryPath').getPath(state.path, field, nextLinkedID);
    if (recordID.lastIndexOf('event:') !== -1) {
      //console.log('events object properties (e.g. meetings) is created here...')
    }

//************************************************************************************************************* */
//here we are creating the new record in store, and assign the __dataID__ to it (Person:2)
    this.createRecordIfMissing(field, nextLinkedID, path, fieldData);
//we then assign myself to my parent's storage key. i.e. Store:1.person{status:"in_progress", dummy:"passed-from-parent"} = Person:2
    this._writer.putLinkedRecordID(recordID, storageKey, nextLinkedID);
//************************************************************************************************************* */
