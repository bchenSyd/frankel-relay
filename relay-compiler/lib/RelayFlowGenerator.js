...
function generate(node) {
  var babelAST = require('./RelayIRVisitor').visit(node, RelayCodeGenVisitor);
...
function makeProp(_ref, concreteType) {
  ...
  var typeProperty = require('babel-types').objectTypeProperty(require('babel-types').identifier(key), value);
  if (conditional) {
...
function selectionsToBabel(selections) {
  ...
  if ((0, _keys2['default'])(byConcreteType).length && onlySelectsTypename([].concat((0, _toConsumableArray3['default'])(baseFields.values()))) && (hasTypenameSelection([].concat((0, _toConsumableArray3['default'])(baseFields.values()))) || (0, _keys2['default'])(byConcreteType).every(function (type) {
    return hasTypenameSelection(byConcreteType[type]);
  }))) {
    var _loop = function _loop(concreteType) {
      types.push(require('babel-types').objectTypeAnnotation([].concat((0, _toConsumableArray3['default'])([].concat((0, _toConsumableArray3['default'])(baseFields.values())).map(function (selection) {
    ...
    var otherProp = require('babel-types').objectTypeProperty(require('babel-types').identifier('__typename'), stringLiteralTypeAnnotation('%other'));
    ...
    types.push(require('babel-types').objectTypeAnnotation([otherProp]));
  } else {
    var selectionMap = selectionsToMap([].concat((0, _toConsumableArray3['default'])(baseFields.values())));
    for (var concreteType in byConcreteType) {
    ...
    types.push(require('babel-types').objectTypeAnnotation((0, _from2['default'])(selectionMap.values()).map(function (sel) {
      return makeProp(sel);
    })));
  }

  if (!types.length) {
    return require('babel-types').objectTypeAnnotation([]);
  }
...
function arrayOfType(thing) {
  return require('babel-types').genericTypeAnnotation(require('babel-types').identifier('Array'), require('babel-types').typeParameterInstantiation([thing]));
}
